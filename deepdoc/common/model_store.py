...
"""Model artifact resolution for local files or remote model downloads."""
...
def _normalize_provider(provider: str | None) -> str:
    ...
    if normalized not in {"auto", "local", "huggingface", "modelscope"}:
        raise ValueError("Unsupported model provider '{}'. Use one of: auto, local, huggingface, modelscope.".format(normalized))
    return normalized
...
class BundleSpec:
    ...
    modelscope_revision_env: str
    modelscope_revision_default: str = "master"
    huggingface_repo_env: str = ""
    huggingface_repo_default: str = "Xorbits/deepdoc"
    huggingface_revision_env: str = ""
    huggingface_revision_default: str = "main"

    @property
    def repo_env(self) -> str:
        return self.modelscope_repo_env

    @property
    def repo_default(self) -> str:
        return self.modelscope_repo_default

    @property
    def revision_env(self) -> str:
        return self.modelscope_revision_env

    @property
    def revision_default(self) -> str:
        return self.modelscope_revision_default
...
BUNDLES: dict[str, BundleSpec] = {
    "vision": BundleSpec(
        ...
        modelscope_repo_env="DEEPDOC_MODELSCOPE_VISION_REPO",
        modelscope_repo_default="Xorbits/deepdoc",
        modelscope_revision_env="DEEPDOC_MODELSCOPE_VISION_REVISION",
        huggingface_repo_env="DEEPDOC_HUGGINGFACE_VISION_REPO",
        huggingface_repo_default="Xorbits/deepdoc",
        huggingface_revision_env="DEEPDOC_HUGGINGFACE_VISION_REVISION",
    ),
    "xgb": BundleSpec(
        name="xgb",
        subdir="xgb",
        ...
        modelscope_repo_env="DEEPDOC_MODELSCOPE_XGB_REPO",
        modelscope_repo_default="Xorbits/deepdoc",
        modelscope_revision_env="DEEPDOC_MODELSCOPE_XGB_REVISION",
        huggingface_repo_env="DEEPDOC_HUGGINGFACE_XGB_REPO",
        huggingface_repo_default="Xorbits/deepdoc",
        huggingface_revision_env="DEEPDOC_HUGGINGFACE_XGB_REVISION",
    ),
}
...
def _resolve_modelscope_repo_id(spec: BundleSpec) -> tuple[str, bool]:
    ...
    Precedence:
      ...
      3) per-bundle default (spec.modelscope_repo_default)

    Returns (repo_id, use_shared_download_dir).
    """
    explicit = os.getenv(spec.modelscope_repo_env)
    ...
    repo_id = spec.modelscope_repo_default.strip()
    # If all bundle defaults point at the same repo, treat it as a combined repo and share the download directory.
    default_repos = {bundle.modelscope_repo_default.strip() for bundle in BUNDLES.values()}
...
def _resolve_modelscope_revision(spec: BundleSpec) -> str:
    ...
    explicit = os.getenv(spec.modelscope_revision_env)
    if explicit and explicit.strip():
    ...
    return spec.modelscope_revision_default


def _resolve_huggingface_repo_id(spec: BundleSpec) -> tuple[str, bool]:
    explicit = os.getenv(spec.huggingface_repo_env)
    if explicit and explicit.strip():
        repo_id = explicit.strip()
        shared_repo = os.getenv(GLOBAL_HUGGINGFACE_REPO_ENV)
        use_shared_dir = bool(shared_repo and shared_repo.strip() and shared_repo.strip() == repo_id)
        return repo_id, use_shared_dir
    ...
    repo_id = spec.huggingface_repo_default.strip()
    default_repos = {bundle.huggingface_repo_default.strip() for bundle in BUNDLES.values()}
    use_shared_dir = len(default_repos) == 1 and repo_id in default_repos
    return repo_id, use_shared_dir


def _resolve_huggingface_revision(spec: BundleSpec) -> str:
    explicit = os.getenv(spec.huggingface_revision_env)
    if explicit and explicit.strip():
        return explicit.strip()

    shared = os.getenv(GLOBAL_HUGGINGFACE_REVISION_ENV)
    if shared and shared.strip():
        return shared.strip()

    return spec.huggingface_revision_default
...
def _huggingface_shared_download_dir(model_home: str | None, repo_id: str, revision: str) -> Path:
    base = _model_home_path(model_home)
    return base.joinpath("huggingface", _slugify_repo_path(repo_id), _slugify_repo_path(revision))
...
def _import_huggingface_snapshot_download():
    try:
        ...
        raise RuntimeError("Hugging Face provider requires the 'huggingface-hub' package. Install it or switch DEEPDOC_MODEL_PROVIDER=local.") from exc
...
def _download_huggingface_repo(*, repo_id: str, revision: str, target_dir: Path, offline: bool) -> Path:
    snapshot_download = _import_huggingface_snapshot_download()

    if not repo_id:
        raise RuntimeError(
            "Hugging Face repo id is empty. Set {} or a bundle-specific env like {}.".format(
                GLOBAL_HUGGINGFACE_REPO_ENV,
                "DEEPDOC_HUGGINGFACE_VISION_REPO",
            )
        )
    ...
    snapshot_root = snapshot_download(**kwargs)
    resolved_snapshot = Path(snapshot_root).expanduser().resolve()
    logging.info("Downloaded Hugging Face repo %s@%s to %s", repo_id, revision, resolved_snapshot)
    return resolved_snapshot


def _candidate_remote_providers(provider_name: str) -> list[str]:
    if provider_name == "auto":
        return ["huggingface", "modelscope"]
    if provider_name in {"huggingface", "modelscope"}:
        return [provider_name]
    return []


def _resolve_remote_location(provider_name: str, spec: BundleSpec, model_home: str | None, local_bundle_dir: Path) -> tuple[str, str, Path]:
    if provider_name == "huggingface":
        repo_id, use_shared_dir = _resolve_huggingface_repo_id(spec)
        revision = _resolve_huggingface_revision(spec)
        download_dir = _huggingface_shared_download_dir(model_home, repo_id, revision) if use_shared_dir else local_bundle_dir
        return repo_id, revision, download_dir

    repo_id, use_shared_dir = _resolve_modelscope_repo_id(spec)
    revision = _resolve_modelscope_revision(spec)
    download_dir = _modelscope_shared_download_dir(model_home, repo_id, revision) if use_shared_dir else local_bundle_dir
    return repo_id, revision, download_dir


def _download_remote_repo(provider_name: str, repo_id: str, revision: str, target_dir: Path, offline: bool) -> Path:
    if provider_name == "huggingface":
        return _download_huggingface_repo(repo_id=repo_id, revision=revision, target_dir=target_dir, offline=offline)
    return _download_modelscope_repo(repo_id=repo_id, revision=revision, target_dir=target_dir, offline=offline)
...
def resolve_bundle_dir(
    ...
    *,
    ...
    """Resolve a model bundle directory from local files or remote providers."""
    ...
    if not offline_mode:
        for remote_provider in _candidate_remote_providers(provider_name):
            repo_id, revision, download_dir = _resolve_remote_location(remote_provider, spec, model_home, local_bundle_dir)
            if download_dir not in roots_to_scan and download_dir != local_bundle_dir:
                roots_to_scan.append(download_dir)
    ...
    for remote_provider in _candidate_remote_providers(provider_name):
        repo_id, revision, download_dir = _resolve_remote_location(remote_provider, spec, model_home, local_bundle_dir)
        try:
            snapshot_root = _download_remote_repo(
                remote_provider,
                repo_id,
                revision,
                download_dir,
            ...
            failures.append(
                "Downloaded {} repo '{}@{}' for bundle '{}' but could not locate the required files under '{}' or '{}'.".format(
                    "Hugging Face" if remote_provider == "huggingface" else "ModelScope",
                    repo_id,
                    revision,
                    spec.name,
                    download_dir,
            ...
            failures.append("{}: {}".format(remote_provider, exc))
            if provider_name != "auto":
                raise

    raise FileNotFoundError("Failed to resolve bundle '{}'. {}".format(spec.name, " ".join(failures)))
